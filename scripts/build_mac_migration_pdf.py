...
# ── Stiller ──
ST_TITLE = ParagraphStyle("title", fontName="Helvetica-Bold", fontSize=22, leading=26, textColor=DARK, spaceAfter=4)
ST_LEAD = ParagraphStyle("lead", fontName="Helvetica", fontSize=11, leading=15, textColor=TEXT, spaceAfter=4)
ST_META = ParagraphStyle("meta", fontName="Courier", fontSize=8, leading=11, textColor=MUTED, spaceAfter=14)
ST_LOGO = ParagraphStyle("logo", fontName="Courier-Bold", fontSize=14, leading=16, textColor=ACCENT, spaceAfter=2)
...
ST_H3 = ParagraphStyle(
    "h3", fontName="Helvetica-Bold", fontSize=11, leading=14, textColor=TEXT, spaceBefore=8, spaceAfter=3
)

ST_BODY = ParagraphStyle(
    "body", fontName="Helvetica", fontSize=10, leading=14, textColor=TEXT, alignment=TA_LEFT, spaceAfter=4
)
ST_BULLET = ParagraphStyle(
    "bullet", parent=ST_BODY, leftIndent=14, bulletIndent=2, bulletFontName="Helvetica-Bold", bulletFontSize=10
)
...
ST_FOOTER = ParagraphStyle("footer", fontName="Courier", fontSize=8, leading=11, textColor=MUTED, alignment=1)
        ...
        for c in row:
            if r_idx == 0:
                style = ParagraphStyle(
                    "th", fontName="Courier-Bold", fontSize=8.5, leading=11, textColor=HexColor("#ffffff")
                )
            else:
                style = ParagraphStyle("td", fontName="Helvetica", fontSize=8.8, leading=11.5, textColor=TEXT)
...
def build():
    ...
    story.append(Paragraph("Windows PC'den macOS'e taşıma — sıfırdan kurulum + dosya transferi + doğrulama", ST_LEAD))
    story.append(
        Paragraph(
    ...
    story += _table(
        ...
        [
            [".env", "C:\\Users\\levent.can\\Projects\\ittracker\\.env", "Secret keys, SMTP, O365 config"],
            ["ms_cer_secret.txt", "%USERPROFILE%\\Documents\\ms_cer_secret.txt", "O365 OAuth secret"],
            ["it_tracker.db", "...\\ittracker\\instance\\it_tracker.db", "Lokal dev DB (opsiyonel)"],
            [
                "MEMORY.md",
    ...
    story += _tip(
        "<b>Aktarım yöntemi:</b> AirDrop, iCloud Drive, USB veya OneDrive. "
    ...
    story += _tip(
        "Kurulum sonunda terminal'in <font face='Courier'>brew</font>'u tanıması için "
    ...
    story += _tip(
        "<b>Bash uyarısı:</b> Mac'te venv aktivasyonu Windows'tan farklı: "
    ...
    story += _p(
        "Windows'tan getirdiğin <font face='Courier'>to-mac</font> klasörünü Mac'te "
    ...
    story += _code("cd ~/Projects/ittracker\nsource venv/bin/activate\npytest -q")
    ...
    story += _p("Anthropic'in resmi kurulum talimatına bak: <font face='Courier'>https://claude.com/claude-code</font>")

    story += _h3("5.1 MEMORY.md transferi (önemli)")
    story += _p(
        "Bu dosya tüm v4.1→v5.0 sürüm notlarını + mimari kararlarını içerir. "
    ...
    story += _tip(
        "<b>Path Key:</b> Claude Code project path'ini &quot;/&quot; → &quot;-&quot; eşlemesiyle "
    ...
    story += _p(
        "Prod sunucu <font face='Courier'>leventcan@10.34.0.62</font>. Mac'ten deploy istersen yeni SSH key gerek."
    )
    ...
    story += _p(
        "<font face='Courier'>deploy.bat</font> Windows-spesifik. Mac'te aynı işi yapan "
        "<font face='Courier'>deploy.sh</font> yazılması gerekir. Şimdilik manuel:"
    )
    story += _code("git push github main\nssh ittracker-prod 'cd ~/ittracker && ./pull-and-rebuild.sh'")
    story += _tip(
        "İstersen <font face='Courier'>deploy.sh</font> Mac portunu birlikte yazarız — "
    ...
    story += _table(
        ["Belirti", "Çözüm"],
        [
            ["cairo / WeasyPrint hatası", "brew install cairo pango gdk-pixbuf libffi · brew link --force libffi"],
            [
                "ModuleNotFoundError: No module named 'X'",
                "venv aktif değil. source venv/bin/activate sonra pip install -r requirements.txt",
            ],
            ["SQLite DB locked", "Başka python app.py instance'ı çalışıyor. ps aux | grep python · kill"],
            [
            ...
            ["Türkçe karakter bozuk (terminal)", "export LANG=tr_TR.UTF-8 (zsh için ~/.zshrc'ye ekle)"],
            ["git push reject", "git pull github main · sonra tekrar git push"],
        ],
